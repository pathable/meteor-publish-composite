...
function publishComposite(name, options) {
    return Meteor.publish(name, function publish(...args) {
        if (options && options.unblock) {
            if (!this.unblock) {
                console.warn('this.unblock is not defined', this);
            } else {
                this.unblock();
            }
        }

        const optionsForInstances = _.omit(options, 'unblock');

        const subscription = new Subscription(this);
        const instanceOptions = prepareOptions.call(this, optionsForInstances, args);
        const publications = [];
